...
float angle = 		LAUNCHER_DEG0;
float angle_prev = 	LAUNCHER_DEG0;
float pole = 		LAUNCHER_POLE;
int angle_des = 	LAUNCHER_ANGLE_DEG;
...
void move_launcher() {
	if (abs(angle - LAUNCHER_ANGLE_DEG) > 0.001) {
		angle = pole * angle_prev + (1 - pole) * angle_des;
		angle_prev = angle;
	}
}
